...
def _generate_content_with_retry(client: genai.Client, prompt: str, max_retries: int = 4, base_delay: int = 10):
    """Gemini 무료 티어는 '503 UNAVAILABLE / high demand' 같은 일시적 과부하 에러가 잦다.
    이런 일시적 에러(503/429/UNAVAILABLE/RESOURCE_EXHAUSTED)만 지수 백오프로 재시도하고,
    그 외의 에러(잘못된 요청 등 재시도해도 안 되는 것)는 바로 위로 던진다."""
    transient_markers = ("UNAVAILABLE", "503", "RESOURCE_EXHAUSTED", "429", "DEADLINE_EXCEEDED")
    last_err = None
