...
def test_save_events_upserts_only_rows_with_valid_title_and_date():
    fake_client = MagicMock()
    topic = {"id": "version_update", "calendar_category": "update"}
    events = [
    ...
    assert upserted[0]["title"] == "6.4 버전 업데이트"
    assert upserted[0]["event_date"] == "2026-08-01"
    assert upserted[0]["source"] == "search"


def test_save_events_skips_upsert_when_no_valid_rows():
    fake_client = MagicMock()
...
# 제목만 다르게(Gemini 비결정성) 다시 찾아내면 그대로 또 insert돼서 중복 행이 쌓였다. select로 미리
# 걸러내는 방식은 같은 실행 한 번 안에서 생기는 중복은 못 잡아서, DB 유니크 인덱스 + upsert로
# 바꿨다(schema.sql 참고) — save_events()는 그 인덱스 컬럼 그대로 on_conflict를 지정하고
# ignore_duplicates=True로 호출하기만 하면 된다.


def test_save_events_upserts_with_dedup_index_columns_and_ignore_duplicates():
    fake_client = MagicMock()
    ...
    call = fake_client.table.return_value.upsert.call_args
    assert call.kwargs["on_conflict"] == "game_id,topic_id,event_date,source_url"
